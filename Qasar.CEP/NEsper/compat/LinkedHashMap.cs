...
namespace com.espertech.esper.compat
{
    ...
    [Serializable]
    public class LinkedHashMap<K, V> : Map<K, V>, ISerializable
    {
        /// <summary>
        ...
        private readonly LinkedList<Pair<K, V>> m_hashList;
        ...
        private readonly HashMap<K, LinkedListNode<Pair<K, V>>> m_hashTable;
        ...
        public LinkedHashMap()
        {
            ...
            this.m_hashTable = new HashMap<K, LinkedListNode<Pair<K, V>>>();
        }
        ...
        public LinkedHashMap(int hashCapacity)
        {
            this.m_hashList = new LinkedList<Pair<K, V>>();
            this.m_hashTable = new HashMap<K, LinkedListNode<Pair<K, V>>>(hashCapacity);
        }
        ...
        {
            this.m_shuffleOnAccess = info.GetBoolean("_shuffle");
            this.m_hashList = info.GetValue("_hashList", typeof (LinkedList<Pair<K, V>>)) as LinkedList<Pair<K, V>>;
            if ( this.m_hashList == null ) {
                throw new SerializationException("unable to deserialize hashList");
            }

            this.m_hashTable = new HashMap<K, LinkedListNode<Pair<K, V>>>((m_hashList.Count*3)/2);
            for (LinkedListNode<Pair<K, V>> node = m_hashList.First; node != null; node = node.Next) {
                m_hashTable.Add(node.Value.First, node);
            }
        }
        ...
        {
            info.AddValue("_hashList", m_hashList);
            info.AddValue("_shuffle", m_shuffleOnAccess);
        }

        #region Map<K,V> Members
        ...
        public V this[K key]
        {
            ...
            set
            {
                LinkedListNode<Pair<K, V>> linkedListNode = m_hashTable.Get(key);
                ...
                else
                {
                    ...
                    m_hashTable[key] = linkedListNode;
                }
