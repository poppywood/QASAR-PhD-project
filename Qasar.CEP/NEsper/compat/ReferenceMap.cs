...
{
    public sealed class ReferenceMap<TKey, TValue> : Map<TKey, TValue>
        where TKey : class
        where TValue : class
    {
        /// <summary>
        ...
        public ReferenceMap( int capacity,
                                    ...
                                    ReferenceType valueReferenceType )
        {
            this.pruneList = new List<object>();
            ...
            this.dictionary = new Dictionary<object, object>(capacity, keyAdapter);
        }
        ...
        public void Add(TKey key, TValue value)
        {
            ...
            Object dictKey = keyAdapter.ReferenceToDictionary(key);
            // Create the dictionary value
            Object dictValue = valueAdapter.ReferenceToDictionary(value);
        ...
        public bool TryGetValue(TKey key, out TValue value)
        {
        	...
        	if ( dictionary.TryGetValue( tempKey, out tempOut ) )
        	{
        	    if (valueAdapter.DictionaryToReference(tempOut, out value))
        	    {
        	        return true;
        	    }
        ...
        public IEnumerator<KeyValuePair<TKey, TValue>> GetEnumerator()
        {
            foreach (KeyValuePair<object, object> entry in dictionary)
            {
                ...
                if ((!keyAdapter.DictionaryToReference(entry.Key, out entryKey)) ||
                    (!valueAdapter.DictionaryToReference(entry.Value, out entryValue)))
                {
                    Prune(entry.Key);
                }
                else
        ...
        public void Prune()
        {
            foreach (Object item in pruneList)
            {
        ...
        #region Map<TKey,TValue> Members
