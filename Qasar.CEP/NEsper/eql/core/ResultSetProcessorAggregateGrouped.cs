...
namespace net.esper.eql.core
{
    ...
    public class ResultSetProcessorAggregateGrouped : ResultSetProcessor
    {
        ...
        /// Aggregation functions in the having node must have been pointed to the AggregationService for evaluation.</param>
			...
			bool isNewData)
        {
            if (isOutputLimiting && !isOutputLimitLastOnly)
            {
                ...
                {
                    result[countTwo++] = groupReps.Get(key, null);
                }
                ...
                if (isSorting)
                {
                    ...
                    foreach (MultiKeyUntyped key in notUpdatedKeys)
                    {
                        sortGenerators[countFour++] = generators.Get(key, null);
                    }
        ...
        private IEnumerator<EventBean> GetGroupedEnumerator(Viewable parent)
        {
        ...
        public IEnumerator<EventBean> GetEnumerator(Viewable parent)
        {
            ...
            {
                return GetGroupedEnumerator(parent);
            }
