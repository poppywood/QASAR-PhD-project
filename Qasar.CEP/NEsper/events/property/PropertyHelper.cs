...
namespace com.espertech.esper.events.property
{
    ...
    public class PropertyHelper
    {
		...
		private static IList<EventPropertyDescriptor> GetPropertiesForClasses(IEnumerable<Type> propertyClasses)
        {
        ...
        {
            PropertyResolver resolver = PropertyResolver.Current;

            // Get the standard set of properties that are returned by
            // the TypeDescriptor.

            foreach (PropertyDescriptor property in resolver.GetProperties(type))
            {
                result.Add(new EventPropertyDescriptor(property.Name, property.Name, property, EventPropertyType.SIMPLE));
            }

            // Get the simple fields
            //foreach ( FieldInfo fieldInfo in GetSimpleFields( type ) ) 
            //{
            //    PropertyDescriptor property = resolver.GetPropertyFor(fieldInfo);
            ...
            foreach (MethodInfo methodInfo in GetSimpleAccessors(type))
            {
            	...
            	PropertyDescriptor property = new FastAccessorPropertyDescriptor( propertyName, methodInfo ) ;
                result.Add( new EventPropertyDescriptor( propertyName, propertyName, property, EventPropertyType.SIMPLE ) ) ;
            }
        }
