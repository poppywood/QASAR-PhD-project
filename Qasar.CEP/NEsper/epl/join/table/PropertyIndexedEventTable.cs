...
namespace com.espertech.esper.epl.join.table
{
    ...
    public class PropertyIndexedEventTable : EventTable
    {
        ...
        protected readonly Map<MultiKeyUntyped, Set<EventBean>> propertyIndex;
        ...
        public virtual void Add(IEnumerable<EventBean> events)
        {
            if (events == null)
            {
                return;
        ...
        public virtual void Remove(IEnumerable<EventBean> events)
        {
            if (events == null)
            {
                return;
		...
		private void Add( EventBean _event )
        {
            ...
            Set<EventBean> events = propertyIndex.Get(key, null);
        ...
        private void Remove(EventBean _event)
        {
         ...
         public bool IsEmpty
         {
             get { return propertyIndex.Count == 0; }
         }

         public IEnumerator<EventBean> GetEnumerator()
         {
             foreach( KeyValuePair<MultiKeyUntyped, Set<EventBean>> entry in propertyIndex )
             {
                 foreach( EventBean eventBean in entry.Value )
                 {
