...
namespace com.espertech.esper.epl.join.plan
{
	...
	public class QueryPlanIndex
	{
		...
		public virtual int GetIndexNum( String[] indexFields )
		{
            ...
            Array.Copy(indexFields, 0, copyIndexFields, 0, indexFields.Length);
            Array.Sort(copyIndexFields);

            for (int i = 0; i < indexProps.Length; i++)
            {
                String[] copyIndexProps = new String[indexProps[i].Length];
                Array.Copy(indexProps[i], 0, copyIndexProps, 0, copyIndexProps.Length);
                Array.Sort(copyIndexProps);

                if ( CollectionHelper.AreEqual( copyIndexFields, copyIndexProps ) )
                {
                    return i;
                }
            }
