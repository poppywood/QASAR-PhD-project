...
namespace com.espertech.esper.epl.join.plan
{
	...
	public class NStreamQueryPlanBuilder
	{
	    ...
	    internal static TableLookupPlan CreateLookupPlan(
			QueryGraph queryGraph,
			...
			EventType[] typesPerStream)
	    {
			...
			if (indexedStreamIndexProps != null)
			{
                ...
                if (indexNum == -1)
                {
                    throw new IllegalStateException("Failed to query plan as index for " + CollectionHelper.Render(indexedStreamIndexProps) + " could looked up in the index specification");
                }
        ...
        /// The method runs through all possible permutations of lookup path <see cref="NumberSetPermutationEnumeration"/>
