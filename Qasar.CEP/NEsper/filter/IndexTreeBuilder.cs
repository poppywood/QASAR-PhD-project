...
{
	/// <summary>
    /// Builder manipulates a tree structure consisting of <see cref="FilterHandleSetNode"/> and <see cref="FilterParamIndexBase" />
    /// instances. Filters can be added to a top node (an instance of FilterHandleSetNode) via the add method. This
    /// method returns an instance of <see cref="IndexTreePath"/> which represents the tree path (list of indizes) that the
    ...
    public sealed class IndexTreeBuilder
    {
        ...
        private bool RemoveFromNode(FilterHandleSetNode currentNode,
                                    IndexTreePath treePathInfo)
        {
            ...
            if (nextPair == null)
            {
                using (new WriterLock(currentNode.NodeRWLock))
                {
                    bool isRemoved = currentNode.Remove(filterHandle);
            ...
            using (new WriterLock(currentNode.NodeRWLock))
            {
                bool _isEmpty = RemoveFromIndex(nextIndex, treePathInfo, filteredForValue);
                ...
                bool isEmptyNode = currentNode.IsEmpty;
                return isEmptyNode;
            }
        }
                                        ...
                                        Object filterForValue)
        {
            using (new WriterLock(index.ReadWriteLock))
            {
                ...
                if (nextPair == null)
                {
                    log.Fatal(".RemoveFromIndex Expected an inner index to this index, this=" + this);
                    return false;
                }
                ...
                {
                    log.Fatal(
                        ".RemoveFromIndex Expected an index for filterHandle that differs from the found index, this=" + this +
                                ...
                                IndexTreePath treePathInfo)
        {
            ...
            using (new ReaderLock(index.ReadWriteLock))
            {
            ...
            WriteIndex(index, filterForValue, treePathInfo);
        }

	    private void WriteIndex(FilterParamIndexBase index, object filterForValue, IndexTreePath treePathInfo)
	    {
	        EventEvaluator eventEvaluator;
	        using (new WriterLock(index.ReadWriteLock))
	        {
