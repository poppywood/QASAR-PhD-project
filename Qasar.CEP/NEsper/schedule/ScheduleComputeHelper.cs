    ...
    public sealed class ScheduleComputeHelper
	{
		...
		private static DateTime Compute(ScheduleSpec spec, long afterTimeInMillis)
		{
			while( true )
			{
	            ...
	            TreeSet<Int32> minutesSet = spec.UnitValues.Get(ScheduleUnit.MINUTES);
	            TreeSet<Int32> hoursSet = spec.UnitValues.Get(ScheduleUnit.HOURS);
	            TreeSet<Int32> monthsSet = spec.UnitValues.Get(ScheduleUnit.MONTHS);
				...
				return GetTime(result, after.Year);
			}
		}
		...
		{
            TreeSet<Int32> daysOfMonthSet = spec.UnitValues.Get(ScheduleUnit.DAYS_OF_MONTH);
            TreeSet<Int32> daysOfWeekSet = spec.UnitValues.Get(ScheduleUnit.DAYS_OF_WEEK);
            TreeSet<Int32> secondsSet = spec.UnitValues.Get(ScheduleUnit.SECONDS);
            TreeSet<Int32> minutesSet = spec.UnitValues.Get(ScheduleUnit.MINUTES);
            TreeSet<Int32> hoursSet = spec.UnitValues.Get(ScheduleUnit.HOURS);
