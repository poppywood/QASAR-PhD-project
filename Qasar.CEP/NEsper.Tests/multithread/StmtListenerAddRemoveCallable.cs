...
namespace net.esper.multithread
{
	...
	{
	    private static long idAllocator;
	    ...
	    {
	        this.id = Interlocked.Increment(ref idAllocator);
	        this.engine = engine;
	        this.stmt = stmt;
	        this.isEQL = isEQL;
	    ...
	    public Object Call()
	    {
	        try
	        {
	            for (int loop = 0; loop < numRepeats; loop++)
	            {
	                ...
	                Assert.IsTrue(found, "event was not found during iteration {0}:{1}", id, loop);
	                ...
	                Assert.IsFalse(found, "event was found, but should not have been");
	            }
	        }
