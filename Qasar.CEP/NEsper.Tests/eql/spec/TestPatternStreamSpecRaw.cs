...
namespace net.esper.eql.spec
{
	...
	public class TestPatternStreamSpecRaw
	{
	    ...
	    public void testPatternExpressions()
	    {
	        ...
	        Assert.AreEqual(typeof(SupportBean), spec.TaggedEventTypes.Get("s").UnderlyingType);

            EvalNodeAnalysisResult evalNodeAnalysisResult = EvalNode.RecursiveAnalyzeChildNodes(spec.EvalNode);
            IList<EvalFilterNode> filters = evalNodeAnalysisResult.FilterNodes;
	    ...
	    private void TryPatternEquals(String text)
	    {
	        ...
	        Assert.AreEqual(typeof(SupportBean), spec.TaggedEventTypes.Get("s").UnderlyingType);
	        Assert.AreEqual(typeof(SupportBean), spec.TaggedEventTypes.Get("t").UnderlyingType);
	    ...
	    {
            PatternStreamSpecCompiled compiled = (PatternStreamSpecCompiled)raw.Compile(SupportEventAdapterService.Service, new MethodResolutionServiceImpl(new EngineImportServiceImpl()), null);
            return compiled;
	    }
