    ...
    public class TestSubselectUnfiltered
    {
        ...
        public void testInvalidSubselect()
        {
            ...
            TryInvalid("select (select (select id from S1.std:lastevent()) id from S1.std:lastevent()) as idS1 from S0",
                       "unexpected token: [\"id\",<158>,line=1,col=52] near line 1, column 52 [select (select (select id from S1.std:lastevent()) id from S1.std:lastevent()) as idS1 from S0]");
            ...
            TryInvalid("select id in (select * from S1.win:length(1000)) as value from S0",
                       "Error starting view: Implicit conversion from datatype '" + typeof(SupportBean_S1).FullName + "' to '" + typeof(int?).FullName + "' is not allowed [select id in (select * from S1.win:length(1000)) as value from S0]");
        }
