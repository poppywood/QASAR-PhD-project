	...
	public class TestPatternGuardPlugIn
	{
	    ...
	    public void testInvalid()
	    {
	        try
	        {
	            ...
	            configuration.AddPlugInPatternGuard("namespace", "name", typeof(Object).FullName);
	            epService = EPServiceProviderManager.GetDefaultProvider(configuration);
	            epService.Initialize();
	        ...
	        {
                Assert.AreEqual("Failed to resolve pattern object: Error casting guard factory instance to net.esper.pattern.guard.GuardFactory interface for guard 'name' [select * from pattern [every net.esper.support.bean.SupportBean where namespace:name(10)]]", ex.Message);
	        }
	    }
	}
} // End of namespace
