...
namespace net.esper.support.util
{
     ...
     * @param expectedValues is the expected values
     */
        public static void AreEqualExactOrder(IEnumerator<EventBean> iterator, String[] fields, Object[][] expectedValues)
        {
            List<Object[]> rows = new List<Object[]>();
            while (iterator.MoveNext())
            {
                EventBean _event = iterator.Current;
                Object[] eventProps = new Object[fields.Length];
                for (int i = 0; i < fields.Length; i++)
                {
                    eventProps[i] = _event.Get(fields[i]);
                }
                rows.Add(eventProps);
            }
