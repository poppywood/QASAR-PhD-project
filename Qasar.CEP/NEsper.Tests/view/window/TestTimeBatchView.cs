...
namespace net.esper.view.window
{
	...
	public class TestTimeBatchView
	{
	    ...
	    public void testViewPushNoRefPoint()
	    {
	        ...
	        Assert.IsTrue(schedulingServiceStub.GetAdded().Get(TEST_INTERVAL_MSEC) != null);
	        ...
	        Assert.IsTrue(schedulingServiceStub.GetAdded().Count == 1);
	        Assert.IsTrue(schedulingServiceStub.GetAdded().Get(TEST_INTERVAL_MSEC - 500) != null);
	        schedulingServiceStub.GetAdded().Clear();
	        ArrayAssertionUtil.AreEqualExactOrder(myView.GetEnumerator(),new EventBean[]{events.Get("c1")});
	        SupportViewDataChecker.CheckOldData(childView, null);
	        SupportViewDataChecker.CheckNewData(childView, null);  // Data got batched, no data release till later
	        ...
	        Assert.IsTrue(schedulingServiceStub.GetAdded().Count == 0);
	        ArrayAssertionUtil.AreEqualExactOrder(myView.GetEnumerator(),new EventBean[]{events.Get("d1")});
	        SupportViewDataChecker.CheckOldData(childView, null);
	        SupportViewDataChecker.CheckNewData(childView, null);
	    ...
	    public void testViewPushWithRefPoint()
	    {
	        ...
	        Assert.IsTrue(schedulingServiceStub.GetAdded().Count == 1);
	        Assert.IsTrue(schedulingServiceStub.GetAdded().Get(1505L) != null);
	        schedulingServiceStub.GetAdded().Clear();
	        ArrayAssertionUtil.AreEqualExactOrder(myView.GetEnumerator(),new EventBean[]{events.Get("A1"), events.Get("A2"), events.Get("A3")});
	        SupportViewDataChecker.CheckOldData(childView, null);
	        SupportViewDataChecker.CheckNewData(childView, null);  // Data got batched, no data release till later
