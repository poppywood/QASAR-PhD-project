...
namespace net.esper.events
{
	...
	public class TestBeanEventType
	{
	    ...
	    public void SetUp()
	    {
            ...
            eventTypeSimple = new BeanEventType(typeof(SupportBeanSimple), beanEventTypeFactory, null, "1");
            eventTypeComplex = new BeanEventType(typeof(SupportBeanComplexProps), beanEventTypeFactory, null, "2");
            eventTypeNested = new BeanEventType(typeof(SupportBeanCombinedProps), beanEventTypeFactory, null, "3");
	    ...
	    public void testGetGetter()
	    {
	        ...
	        try
	        {
	            ...
	            EventType type = beanEventTypeFactory.CreateBeanType(typeof (Object).FullName, typeof (Object));
                EventBean eventBean = new BeanEventBean(new Object(), type);
	            getter.GetValue(eventBean);
	            Assert.Fail();
	        }
	    ...
	    public void testProperties()
	    {
            ...
            tests.Add(new PropTestDesc("array[1].mapprop", true, typeof(EDictionary<String, SupportBeanCombinedProps.NestedLevTwo>), true, objCombined.GetIndexed(1).GetMapProp()));
            ...
            tests.Add(new PropTestDesc("indexed[1].mapprop", true, typeof(EDictionary<String, SupportBeanCombinedProps.NestedLevTwo>), true, objCombined.GetIndexed(1).GetMapProp()));
	    ...
	    {
            BeanEventType type = new BeanEventType(typeof(ISupportAImplSuperGImplPlus), beanEventTypeFactory, null, "1");
	        ...
	        ArrayAssertionUtil.AreEqualAnyOrder(
                    ...
                    beanEventTypeFactory.CreateBeanType("e1", typeof(ISupportAImplSuperG)),
                    beanEventTypeFactory.CreateBeanType("e2", typeof(ISupportBaseAB)),
                    beanEventTypeFactory.CreateBeanType("e3", typeof(ISupportA)),
                    beanEventTypeFactory.CreateBeanType("e4", typeof(ISupportB)),
                    beanEventTypeFactory.CreateBeanType("e5", typeof(ISupportC))	                });
	    }
	    ...
	    {
            eventTypeSimple = new BeanEventType(typeof(ISupportAImplSuperGImplPlus), beanEventTypeFactory, null, "1");

	        List<Type> superTypes = new List<Type>();
            foreach( EventType eventType in eventTypeSimple.SuperTypes)
            {
                superTypes.Add( eventType.UnderlyingType);
            }

	        Assert.AreEqual(5, superTypes.Count);
	        Assert.IsTrue(superTypes.Contains(typeof (ISupportAImplSuperG)));
	        Assert.IsTrue(superTypes.Contains(typeof (ISupportB)));
	        Assert.IsTrue(superTypes.Contains(typeof (ISupportC)));

            eventTypeSimple = new BeanEventType(typeof(Object), beanEventTypeFactory, null, "2");
	        List<EventType> eventSuperTypes = new List<EventType>(eventTypeSimple.SuperTypes);
	        Assert.AreEqual(0, eventSuperTypes.Count);

            BeanEventType type = new BeanEventType(typeof(ISupportD), beanEventTypeFactory, null, "3");
