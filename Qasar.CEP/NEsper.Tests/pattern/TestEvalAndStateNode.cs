...
namespace net.esper.pattern
{
	...
	public class TestEvalAndStateNode
	{
	    ...
	    public void testGenerate()
	    {
	        ...
	        Assert.IsTrue(result[0].GetMatchingEvent("0") == events.Get("0"));

	        Assert.IsTrue(result[0].GetMatchingEvent("a") == events.Get("a"));
	        Assert.IsTrue(result[0].GetMatchingEvent("c") == events.Get("c"));
	        Assert.IsTrue(result[0].GetMatchingEvent("e") == events.Get("e"));

	        Assert.IsTrue(result[1].GetMatchingEvent("0") == events.Get("0"));
	        Assert.IsTrue(result[1].GetMatchingEvent("a") == events.Get("a"));
	        Assert.IsTrue(result[1].GetMatchingEvent("c") == events.Get("c"));
	        Assert.IsTrue(result[1].GetMatchingEvent("f") == events.Get("f"));

	        Assert.IsTrue(result[7].GetMatchingEvent("0") == events.Get("0"));
	        Assert.IsTrue(result[7].GetMatchingEvent("b") == events.Get("b"));
	        Assert.IsTrue(result[7].GetMatchingEvent("d") == events.Get("d"));
	        Assert.IsTrue(result[7].GetMatchingEvent("f") == events.Get("f"));
	    }
