...
namespace net.esper.adapter.csv
{
	...
	public class CSVInputAdapterSpec
	{
		...
		public IDictionary<String, Type> PropertyTypes
		{
            ...
            set
            {
                EDictionary<String,Type> tempDict = new HashDictionary<String, Type>();
                foreach( KeyValuePair<String,Type> entry in value )
                {
                    tempDict[entry.Key] = TypeHelper.GetBoxedType(entry.Value);
                }

                propertyTypes = tempDict;
            }
		}
